...
function readFile(file) {
    ...
    reader.onload = function(progressEvent) {
        ...
        // split by windows new line carriage return
...
function downloadText(text) {
    ...
    pom.setAttribute('download', 'result.csv'); // sets the resultant file to be named "result.txt"
...
function parseData(line, data, locObj, max) {
    ...
    if (Object.keys(locObj).length === max) {
        ...
        for (var key in locObj) {
            // logic to build output goes here
            text += key + "," + locObj[key] + "\r\n";
        }
        text.replace("^M", "");
        downloadText(text);
